...
| C3 | Redemption liquidity: buffer sizing, or a claim-note path if a buffer proves insufficient. Closing a position to fund a redemption is subject to the few-second order delay in §3.3. **Note the trap:** a buffer held on the EVM side is equity that is not posted as margin, so the Core account's real leverage exceeds the vault's headline leverage and its liquidation price moves closer. Liquidation depends only on leverage, not size: `P_liq = P × (1 − 1/L) / (1 − 1/(2·L_max))`. A 2x vault holding 10% of equity as buffer runs 2.22x on Core, shifting liquidation from **−47.4% to −42.1%**. Corollary worth encoding: a **proportional** unwind — closing position and withdrawing margin in the same ratio — leaves L unchanged and is therefore liquidation-neutral. Unwind slightly more than proportional to absorb fees and slippage, which otherwise land the vault above target. Mitigation is to top margin up from the buffer as the position approaches maintenance — class transfers are not delayed, so this is fast — but it must be an explicit rule, not an emergent behaviour. |
