...
import {console2} from "forge-std/console2.sol"; // used in test_transfer4x2WethUsdcWithoutFee
...
/// @dev Tests the transact42 circuit (4 inputs, 2 outputs).
/// Two sequential deposits of 1000 WETH + 1000 USDC each produce 4 input notes;
/// the transfer consumes all 4 and emits 2 output notes (one per asset) to the receiver.
contract PoolTransfer42Test is PoolTest {
    function setUp() public {
        _setUp();
        _registerVerifierTransact42();
        _makePreDeposit4x2A();
        _makePreDeposit4x2B();
    }

    function _registerVerifierTransact42() internal {
    ...
    function test_transfer4x2WethUsdcWithoutFee() external {
        ...
        console2.log(
            "Fixture::transfer_4x2_weth_usdc_without_fee::MerkleRoot:",
    ...
    function test_revertOnDoubleSpend4x2Transfer() external {
        ShieldedTransaction memory stx = _loadShieldedTransaction(
            "transfer_4x2_weth_usdc_without_fee"
        );
        pool.transact(stx);
