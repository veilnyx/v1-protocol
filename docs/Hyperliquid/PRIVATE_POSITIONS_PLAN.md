...
### 2.6 Why did Hyperliquid chose to restrict Multi-Position Leverage and opted for netted account design per asset

Hyperliquid enforces a single net position per asset per account because HyperCore operates as a **central clearinghouse netting model**, not a ticket-based CFD broker.

### Why Multi-Position Leverage is Restricted

* **$O(1)$ Deterministic Risk & Liquidations:** Multiple leverage points on a single account create execution ambiguity during margin calls (e.g., deciding which position to liquidate first). A single net position allows instant, constant-time margin checks.
* **Validator State Compaction:** User accounts use a fixed, compact memory layout (`size`, `entryPx`, `margin`, `leverage`). Variable-length sub-positions per asset would cause memory fragmentation, state bloat, and execution latency.
* **Efficient Funding Settlement:** Funding payments are processed in a single pass ($\text{Net Size} \times \text{Funding Rate}$) rather than iterating through $N$ individual position tickets per user every cycle.
* **True Delta Exposure:** Margin systems evaluate true directional exposure. Splitting positions artificially doubles funding costs, inflates collateral requirements, and increases unnecessary liquidation risk.

### Workarounds for Isolated Strategies

* **Native L1 Sub-Accounts:** Create native sub-accounts from the master wallet. Each functions as an independent 20-byte clearing address with isolated margin and separate leverage settings.
* **HyperEVM Smart Proxies:** Deploy lightweight ERC-1167 proxy contracts. HyperCore treats each smart contract address as an independent trading account on L1.
...
## 10. Shiven's Questions
Q1:Regarding return path: its going to be async right, once the order is executed on Core..How is this delay due to async nature handled on UI
Q2:  Whats being deposited back to Veilnyx? Can you elaborate your understanding of this
Q3: Explain more on position view using precompiles. Whats 0x800? Action ID for position reading in hex or something else?
Q4 - how will the shileded account <> burner account linkage happen in Veilnyx client/onchain to show posiint updates per user?
