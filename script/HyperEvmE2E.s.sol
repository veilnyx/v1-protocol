...
contract HyperEvmE2E is PoolTest {
    function _deployAll(address me) internal {
        ...
        // Real Pool, not MockPool: the point of this script is a production stack.
        ...
        pool = MockPool(payable(address(poolProxy)));  // typed as MockPool for PoolTest helpers; implementation is the real Pool
    ...
    function run() external {
        ...
        // Submit a real treeUpdate proof rather than PoolTest's fabricated one.
        // _processCommitmentTreeQueue builds TreeUpdateData with proof: bytes("")
        // and only passes against a mock verifier, so it cannot be used against a
        // production stack. This fixture proves insertion of the commitments
        // deposit_pre_tx queues, generated by genHyperEvmTreeUpdate.
        pool.updateCommitmentTree(_loadTreeUpdateData("tree_update_deposit_pre_tx"));
